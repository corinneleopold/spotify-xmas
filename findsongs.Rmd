...
### Since `searchPlaylists` returns many playlists, get the number of followers for each playlist and only keep the playlists with the top 5 follower counts.
```{r}
getFollowers <- function(df) {
  ...
  for (id in df$playlist_id) {
    ...
    df$followers[i] <- df$followers$total
    i <- i + 1
  }
