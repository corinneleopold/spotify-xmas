...
```{r}
instrumPL <- searchPlaylists("Christmas instrumental", get_spotify_access_token()) # Instrumental query
head(instrumPL)

acapellaPL <- searchPlaylists("a cappella christmas", get_spotify_access_token()) # A cappella query
head(acapellaPL)
```
...
# getTopPLs(playlistDf, n = 3)
getTopPLs <- function(df, n = 5) {
...
```{r}
topPLs <- getTopPLs(instrumPL, n = 5)
head(topPLs)
```
